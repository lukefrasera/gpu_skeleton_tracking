...
static void GenerateAdjacencyList(SkeltrackSkeleton *self) {
  SkeltrackSkeletonPrivate *priv;
  priv = self->priv;
  ...
  for (current = g_list_first (nodes);
      current != NULL; current = g_list_next (current)) {
    node = (Node*)current->data;
  ...
  for (current = g_list_first (nodes);
    ...
    node = (Node*)current->data;
    int i;
    priv->adjacency_list.vertices[v_index] = e_index;
    GList* neightbor_list = g_list_first(node->neighbors);
    for (i = 0; i < g_list_length(node->neighbors); ++i) {
      // Add neighbors to edge list
      Node * neightbor = (Node *)neightbor_list->data;
      priv->adjacency_list.edges[i + e_index] = neightbor->index;
      neightbor_list = g_list_next(neightbor_list);
    }
    e_index += g_list_length(node->neighbors);
    v_index++;
  }
}
